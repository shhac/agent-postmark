...
`skills/agent-postmark` to `shhac/agent-skills`. **No manual build, and no manual
formula bump.**
...
5. Verify CI and the outputs:
   ```bash
   gh run watch --repo shhac/agent-postmark          # release + Publish skill runs green
...
repo-specific "agent-postmark release automation (env-scoped)" one). The skill
...
```bash
ssh-keygen -t ed25519 -N "" -C "agent-postmark release automation" -f tap_key
gh repo deploy-key add tap_key.pub -R shhac/homebrew-tap --allow-write \
  --title "agent-postmark release automation (env-scoped)"
gh secret set TAP_DEPLOY_KEY --repo shhac/agent-postmark --env homebrew-tap < tap_key
